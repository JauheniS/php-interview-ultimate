...
- **Does adding a public field in an extended class violate LSP?**
  Generally, no. LSP is about behavioral subtyping. However, if the base class is intended to be used in a way that assumes it has *only* certain fields, adding more might complicate its use, but it doesn't strictly break the substitution of the base class.

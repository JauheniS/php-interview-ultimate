...
*Note: In PHP 7+, the `zval` structure was significantly optimized to reduce memory usage and improve performance (e.g., by moving `refcount` and `is_ref` into the value itself for certain types and avoiding heap allocation for simple types).*
